...
(function() {
  ...
  var scramSha1 = factory.use(require('./scram')).create(['SCRAM-SHA-1']);
  var digest = factory.use(require('./digest')).create(['DIGEST-MD5']);
  ...
  var SASLMixin = (function() {
    ...
    return {
      ...
      sasl: function(authOpts, challenge) {
        logger.debugf(authOpts);
        return function () {
          var response;
            switch (authOpts.saslMechanism) {
              case 'PLAIN':
                challengeNum = 1;
                logger.tracef('Try auth with PLAIN');
                currentAuthMech = 'PLAIN';
                response = '';
                response += '\0';
                response += authOpts.userName;
                response += '\0';
                response += authOpts.password;
                return [codec.encodeString('PLAIN'), codec.encodeString(response)];
              case 'SCRAM-SHA-1':
                logger.tracef('Try auth with SCRAM-SHA-1');
                currentAuthMech = 'SCRAM-SHA-1';
                if (challenge == undefined) {
                  // initial challenge
                  logger.tracef('Initial SCRAM-SHA-1 challenge');
                  challengeNum = 1;
                  var initial = scramSha1.response({username: authOpts.userName, password: authOpts.password});
                  return [codec.encodeString('SCRAM-SHA-1'), codec.encodeString(initial)];
                }

                logger.tracef('Second SCRAM-SHA-1 challenge');
                challengeNum = 1;
                var scramSecResp = scramSha1.challenge(challenge)
                    .response({username: authOpts.userName, password: authOpts.password});

                console.log('==========');
                console.log(scramSecResp);

                return  [ codec.encodeString('SCRAM-SHA-1'), codec.encodeString(scramSecResp)];
              case 'DIGEST-MD5':
                logger.tracef('Try auth with DIGEST-MD5');
                ...
                var digestResponse = digest.challenge(challenge)
                    .response({ username: authOpts.userName, qop: null,
                      password: authOpts.password, serviceType: 'hotrod', host: 'infinispan'});
                console.log('==========');
                console.log(digestResponse);
                return  [codec.encodeString(''), codec.encodeString(digestResponse)];
              default:
      ...
      decodeSasl: function(header, bytebuf) {
        ...
        if(authDone == 1) {
          logger.tracef('Auth done, authenticated');
          currentAuthMech = '';
          challengeNum = 0;
          return {result:  {challenge: 0, response: DECODE_UBYTE(bytebuf)}, continue: true}
        }

        logger.tracef('Auth not done, continue for next challenge');

        if (currentAuthMech == 'SCRAM-SHA-1' && challengeNum == 1) {
          logger.tracef('Decode challenge 1');
          return {result:  {challenge: challengeNum, response: DECODE_STRING(bytebuf)}, continue: true}
        }
        if (currentAuthMech == 'DIGEST-MD5' && challengeNum == 1) {
          logger.tracef('Decode challenge 1');
          var challenge = DECODE_STRING(bytebuf)
          return {result:  {challenge: challengeNum, response: challenge}, continue: true}
        }
